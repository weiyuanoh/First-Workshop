...
def get_fff_returns():
    ...
    """
    rets = pd.read_csv("F-F_Research_Data_Factors.CSV",
...
def msr_window_annrets(r, estimation_window):
    n_periods = r.shape[0]
    # return windows
    windows = [(start, start+estimation_window) for start in range(n_periods-estimation_window)]
    data = [r.iloc[win[0]:win[1]] for win in windows]
    # rewriting msr function 
    def msr(er, cov , riskfree_rate):
        n = er.shape[0]
        init_guess = np.repeat(1/n, n)
        bounds = ((0.0, 1.0),) * n # an N-tuple of 2-tuples!
        # construct the constraints
        ...
        return weights.x
    # return weights 
    weights = [msr(annualize_rets(data, periods_per_year=12), cov = sample_cov(data), riskfree_rate=0.0525) for data in data ]
    last_weights = weights[-1]    
    # convert List of weights to DataFrame
    weights = pd.DataFrame(weights, index=r.iloc[estimation_window:].index, columns=r.columns)
   
    returns = (weights * r).sum(axis="columns",  min_count=1) #mincount is to generate NAs if all inputs are NAs
    return (returns, last_weights)


def value(sum, weights):
    '''
    weights --> array of weights(gmv, msr, gmv-cc, etc)
    sum --> int, initial sum invested money
    returns the amount of money invested in each stock based on the array of weights given 
...
def num(value, tickerlist):
    '''
    returns the number of stock to buy for rebalancing based on last close price 
    '''
    lastprice = yfin.download(tickerlist, period='1d')['Adj Close']
    num = value/ lastprice 
    return num 


def ff_analysis_series(tickers, rets, timeframe_start, timeframe_end, ff):
    '''
    does a fama french factor analysis for each stock 
    and returns the results in a dataframe for stock pick purposes 

    '''
    results = pd.DataFrame()
    for stock in rets.columns:
        mkt_excess = rets[timeframe_start:timeframe_end] - ff.loc[timeframe_start:timeframe_end, ['RF']].values
        mkt_excess = mkt_excess.loc[timeframe_start:timeframe_end, [stock]]
    
        mkt_rf = ff.loc[timeframe_start:timeframe_end, ['Mkt-RF', 'SMB', 'HML']]
        y = mkt_excess.copy()
        x = mkt_rf
        x['Constant'] = 1
        r = sm.OLS(y,x).fit()
        df = r.params.to_frame()
        df = df.T
